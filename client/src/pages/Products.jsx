...
export default function Products() {
  ...
  useEffect(() => { setLoading(true); Promise.all([api.get('/products', { params: { search: query, category: category !== 'all' ? category : undefined, sort } }), api.get('/categories')]).then(([productsResponse, categoriesResponse]) => { setProducts(productsResponse.data.data); setCategories(categoriesResponse.data); }).catch(() => { setProducts(demoProducts.filter((product) => { const matchesQuery = !query || `${product.name} ${product.description}`.toLowerCase().includes(query.toLowerCase()); const matchesCategory = category === 'all' || product.category?.slug === category; return matchesQuery && matchesCategory; })); }).finally(() => setLoading(false)); }, [query, category, sort]);
